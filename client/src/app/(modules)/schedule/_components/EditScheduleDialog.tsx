...
export function EditScheduleDialog({ schedule, onClose, onSuccess }: EditScheduleDialogProps) {
  ...
  useEffect(() => {
    if (schedule) {
      ...
      setGoal(schedule.intent_spec?.goal || '');
  ...
  const parseCronExpression = (cron: string) => {
    ...
    if (parts.length >= 5) {
      setMinute(parts[0]);
      setHour(parts[1]);
      if (parts[4] === '1-5') setFrequency('weekday');
      else if (parts[2] === '1' && parts[4] === '*') setFrequency('monthly');
      else if (parts[4] === '1') setFrequency('weekly');
  ...
  const previewText = () => {
    ...
    const labels: Record<Frequency, string> = {
      daily: '每天', weekday: '每个工作日', weekly: '每周一', monthly: '每月1号',
    };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-300">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">编辑定时任务</h2>
          </div>
          <button
            onClick={onClose}
