...
class GraphRuntime:
    ...
    async def resume_from_snapshot(
        ...
        """
        Resume graph execution from the latest persisted snapshot.

        Loads the latest snapshot, restores ExecutionContext, re-executes
        RUNNING nodes (reset to PENDING), and skips SUCCESS nodes.

        Args:
            graph_id: ID of the graph to resume
            state_store: StateStore to load snapshot from
            context: Optional ExecutionContext override

        Returns:
            ExecutionResult from resumed execution

        Requirements: 3.5, 12.6, 12.7, 29.12
        """
        ...
        snapshot = state_store.load_latest(graph_id)
        if snapshot is None:
            return ExecutionResult(
                success=False,
                final_status="failed",
                execution_time=0.0,
                error_message=f"No snapshot found for graph {graph_id}"
            )

        # Restore graph from snapshot
        graph = ExecutionGraph.model_validate_json(snapshot["graph_json"])

        # Reset RUNNING nodes back to PENDING (they were interrupted)
        for node in graph.nodes.values():
            if node.status == NodeStatus.RUNNING:
                node.status = NodeStatus.PENDING
                node.start_time = None
                logger.info(f"[GraphRuntime] Reset interrupted node {node.id} to PENDING")

        # Restore or create ExecutionContext (Requirement 29.12)
        if context is None:
            ctx_data = _json.loads(snapshot.get("context_json", "{}"))
            context = ExecutionContext(
                graph_id=graph_id,
                goal_desc=graph.goal,
                inputs=ctx_data.get("variables", {}),
                session_id=ctx_data.get("session_id"),
                task_id=graph_id,
            )
            # Restore node outputs
            for node_id, outputs in ctx_data.get("node_outputs", {}).items():
                context.set_node_output(node_id, outputs)
            # Restore variables
            context.variables.update(ctx_data.get("variables", {}))

        logger.info(
            f"[GraphRuntime] Resuming graph {graph_id} from snapshot v{snapshot['snapshot_version']} "
            f"({snapshot['nodes_completed']} nodes previously completed)"
        )

        return await self.execute_graph(graph, context)
