...
from app.avatar.runtime.multiagent.execution.task_ownership import TaskOwnershipManager
...
class InstanceManager:
    ...
    def spawn(
        ...
        if spec is None:
            from app.avatar.runtime.multiagent.roles.role_spec import RoleSpec
...
class GraphValidator:
    """Semantic-level subtask graph validator (v2).

    Validates beyond DAG structure:
    1. DAG acyclicity + required fields + schema compliance (existing)
    2. input_bindings have producers (every ref points to an existing upstream node)
    3. output_contract compatibility (upstream output type matches downstream expectation)
    4. Role availability (suggested_role has a registered RoleRunner)
    5. Parallel write conflict detection (parallel nodes writing same format)
    """
    ...
    def validate_rules(self, graph: SubtaskGraph) -> tuple[bool, List[str]]:
        """Full rule-based validation: structural + semantic."""
        all_errors: List[str] = []
        ...
        ok, errors = self._validate_input_binding_producers(graph)
        if not ok:
            all_errors.extend(errors)

        ok, errors = self._validate_role_runners(graph)
        if not ok:
            all_errors.extend(errors)

        ok, errors = self._validate_parallel_write_conflicts(graph)
        if not ok:
            all_errors.extend(errors)

        return (len(all_errors) == 0, all_errors)

    def _validate_input_binding_producers(
        self, graph: SubtaskGraph,
    ) -> tuple[bool, List[str]]:
        """Check that every input_binding reference points to an existing upstream node."""
        errors: List[str] = []
        # Build set of upstream node IDs for each node
        upstream_map: Dict[str, set] = {nid: set() for nid in graph.nodes}
        for edge in graph.edges:
            if edge.target_node_id in upstream_map:
                upstream_map[edge.target_node_id].add(edge.source_node_id)

        for nid, node in graph.nodes.items():
            for binding_name, ref in node.input_bindings.items():
                if not isinstance(ref, str) or "." not in ref:
                    continue
                ref_node_id = ref.split(".", 1)[0]
                if ref_node_id not in graph.nodes:
                    errors.append(
                        f"Node '{nid}' input_binding '{binding_name}' "
                        f"references non-existent node '{ref_node_id}'"
                    )
                elif ref_node_id not in upstream_map.get(nid, set()):
                    errors.append(
                        f"Node '{nid}' input_binding '{binding_name}' "
                        f"references '{ref_node_id}' which is not an upstream dependency"
                    )
    ...
    def _validate_role_runners(
        ...
        """Check that every node's role has a registered RoleRunner."""
        errors: List[str] = []
        try:
            from app.avatar.runtime.multiagent.roles.role_runners import _ROLE_RUNNERS
            available_roles = set(_ROLE_RUNNERS.keys())
        except ImportError:
            return True, []  # Can't validate without role_runners module

        for nid, node in graph.nodes.items():
            if node.responsible_role and node.responsible_role not in available_roles:
                errors.append(
                    f"Node '{nid}' role '{node.responsible_role}' "
                    f"has no registered RoleRunner (available: {sorted(available_roles)})"
                )
        return (len(errors) == 0, errors)
    ...
    def _validate_parallel_write_conflicts(
        ...
        """Detect parallel nodes that may write the same output format.

        Nodes in the same topological layer with is_parallel=True and
        the same output format could cause write conflicts.
        """
        errors: List[str] = []
        parallel_groups = graph.get_parallel_groups()

        for layer_idx, layer_ids in enumerate(parallel_groups):
            if len(layer_ids) <= 1:
                continue
            # Collect output formats for parallel nodes in this layer
            format_to_nodes: Dict[str, List[str]] = {}
            for nid in layer_ids:
                node = graph.nodes[nid]
                if not node.is_parallel:
                    continue
                fmt = (node.output_contract or {}).get("format")
                if fmt:
                    format_to_nodes.setdefault(fmt, []).append(nid)

            for fmt, nodes in format_to_nodes.items():
                if len(nodes) > 1:
                    errors.append(
                        f"Layer {layer_idx}: parallel nodes {nodes} "
                        f"both produce format '{fmt}' — potential write conflict"
                    )
        return (len(errors) == 0, errors)

    async def validate_llm(self, graph: SubtaskGraph) -> tuple[bool, List[str]]:
        """Optional lightweight LLM logic validation. Placeholder for future."""
        return True, []
